...
JOIN NAPOLITANA_CON_FRITAS.BI_Compra_Auto AS C ON C.MES_ID = V.MES_ID AND C.SUCURSAL_ID = V.SUCURSAL_ID
...
JOIN NAPOLITANA_CON_FRITAS.BI_Compra_Autoparte AS C ON C.MES_ID = V.MES_ID AND C.SUCURSAL_ID = V.SUCURSAL_ID
GROUP BY V.MES_ID, V.SUCURSAL_ID

GO

CREATE VIEW NAPOLITANA_CON_FRITAS.VW_Autoparte_Precio_Promedio AS
SELECT V.AUTOPARTE_ID, AVG(V.PRECIO) AS PROMEDIO_VENDIDO, AVG(C.PRECIO) AS PROMEDIO_COMPRADO
FROM NAPOLITANA_CON_FRITAS.BI_Venta_Autoparte AS V
...
SELECT C.SUCURSAL_ID, M.ANIO, MAX(C.CANTIDAD) AS MAXIMO_STOCK
FROM NAPOLITANA_CON_FRITAS.BI_Compra_Autoparte AS C
JOIN NAPOLITANA_CON_FRITAS.BI_MES AS M ON M.ID = C.MES_ID
