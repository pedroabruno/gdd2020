...
POTENCIA_RANGO_ID int NULL,  -- nunca tuvo datos, tiene que poder ser null
...
CLIENTE_ID int,
FABRICANTE_ID decimal(18,0),
RUBRO_AUTOPARTE_ID int NULL, -- no hay datos todavia, tiene que poder ser null
PRECIO decimal(18,2) DEFAULT 0,
CANTIDAD decimal(18,0) DEFAULT 0,
...
INSERT INTO NAPOLITANA_CON_FRITAS.BI_Compra_Autoparte (AUTOPARTE_ID,MES_ID,SUCURSAL_ID,CLIENTE_ID,FABRICANTE_ID,PRECIO,CANTIDAD)
-- falta agregar la referencia a la tabla BI_Rubro_Autoparte que todavia no tiene datos
SELECT IC.ITEM_AUTOPARTE_ID, MES.ID, C.COMPRA_SUCURSAL, 
	(CASE WHEN DATEDIFF(YEAR,CL.CLIENTE_FECHA_NAC,GETDATE()) BETWEEN 18 AND 30 THEN (SELECT TOP 1 A.ID FROM NAPOLITANA_CON_FRITAS.BI_Cliente A WHERE A.SEXO = 'NO INFORMA' AND A.RANGO_EDAD = '18 - 30 años')
		WHEN DATEDIFF(YEAR,CL.CLIENTE_FECHA_NAC,GETDATE()) BETWEEN 31 AND 50 THEN (SELECT TOP 1 A.ID FROM NAPOLITANA_CON_FRITAS.BI_Cliente A WHERE A.SEXO = 'NO INFORMA' AND A.RANGO_EDAD = '31 - 50 años')
		WHEN DATEDIFF(YEAR,CL.CLIENTE_FECHA_NAC,GETDATE()) > 50 THEN (SELECT TOP 1 A.ID FROM NAPOLITANA_CON_FRITAS.BI_Cliente A WHERE A.SEXO = 'NO INFORMA' AND A.RANGO_EDAD = '> 50 años')
		ELSE NULL
	END)
	, M.MODELO_FABRICANTE, AVG(IC.ITEM_PRECIO), SUM(IC.ITEM_CANTIDAD)
...
JOIN NAPOLITANA_CON_FRITAS.Cliente AS CL ON CL.CLIENTE_ID = C.COMPRA_CLIENTE
GROUP BY MES.ID, C.COMPRA_SUCURSAL, C.COMPRA_CLIENTE, M.MODELO_FABRICANTE, IC.ITEM_AUTOPARTE_ID, CL.CLIENTE_FECHA_NAC


INSERT INTO NAPOLITANA_CON_FRITAS.BI_Venta_Autoparte (AUTOPARTE_ID,MES_ID,SUCURSAL_ID,CLIENTE_ID,FABRICANTE_ID,PRECIO,CANTIDAD)
-- falta agregar la referencia a la tabla BI_Rubro_Autoparte que todavia no tiene datos
SELECT ItF.ITEM_AUTOPARTE_ID, MES.ID, F.FACTURA_SUCURSAL, 
	(CASE WHEN DATEDIFF(YEAR,CL.CLIENTE_FECHA_NAC,GETDATE()) BETWEEN 18 AND 30 THEN (SELECT TOP 1 A.ID FROM NAPOLITANA_CON_FRITAS.BI_Cliente A WHERE A.SEXO = 'NO INFORMA' AND A.RANGO_EDAD = '18 - 30 años')
		WHEN DATEDIFF(YEAR,CL.CLIENTE_FECHA_NAC,GETDATE()) BETWEEN 31 AND 50 THEN (SELECT TOP 1 A.ID FROM NAPOLITANA_CON_FRITAS.BI_Cliente A WHERE A.SEXO = 'NO INFORMA' AND A.RANGO_EDAD = '31 - 50 años')
		WHEN DATEDIFF(YEAR,CL.CLIENTE_FECHA_NAC,GETDATE()) > 50 THEN (SELECT TOP 1 A.ID FROM NAPOLITANA_CON_FRITAS.BI_Cliente A WHERE A.SEXO = 'NO INFORMA' AND A.RANGO_EDAD = '> 50 años')
		ELSE NULL
	END)
	, M.MODELO_FABRICANTE, AVG(ItF.ITEM_PRECIO), SUM(ItF.ITEM_CANTIDAD)
...
JOIN NAPOLITANA_CON_FRITAS.Modelo AS M ON AP.AUTOPARTE_MODELO = M.MODELO_CODIGO
JOIN NAPOLITANA_CON_FRITAS.Cliente AS CL ON CL.CLIENTE_ID = F.FACTURA_CLIENTE
GROUP BY MES.ID, F.FACTURA_SUCURSAL, F.FACTURA_CLIENTE, M.MODELO_FABRICANTE, ItF.ITEM_AUTOPARTE_ID, CL.CLIENTE_FECHA_NAC
...
SELECT MES.ID, C.COMPRA_SUCURSAL, 
	(CASE WHEN DATEDIFF(YEAR,CL.CLIENTE_FECHA_NAC,GETDATE()) BETWEEN 18 AND 30 THEN (SELECT TOP 1 A.ID FROM NAPOLITANA_CON_FRITAS.BI_Cliente A WHERE A.SEXO = 'NO INFORMA' AND A.RANGO_EDAD = '18 - 30 años')
		WHEN DATEDIFF(YEAR,CL.CLIENTE_FECHA_NAC,GETDATE()) BETWEEN 31 AND 50 THEN (SELECT TOP 1 A.ID FROM NAPOLITANA_CON_FRITAS.BI_Cliente A WHERE A.SEXO = 'NO INFORMA' AND A.RANGO_EDAD = '31 - 50 años')
		WHEN DATEDIFF(YEAR,CL.CLIENTE_FECHA_NAC,GETDATE()) > 50 THEN (SELECT TOP 1 A.ID FROM NAPOLITANA_CON_FRITAS.BI_Cliente A WHERE A.SEXO = 'NO INFORMA' AND A.RANGO_EDAD = '> 50 años')
		ELSE NULL
	END)
	, M.MODELO_CODIGO, M.MODELO_FABRICANTE, M.MODELO_TIPO_AUTO, M.MODELO_TIPO_CAJA, M.MODELO_TIPO_MOTOR, M.MODELO_TIPO_TRANSMISION, 
...
JOIN NAPOLITANA_CON_FRITAS.Cliente AS CL ON CL.CLIENTE_ID = C.COMPRA_CLIENTE
GROUP BY MES.ID, C.COMPRA_SUCURSAL, C.COMPRA_CLIENTE, M.MODELO_CODIGO, M.MODELO_FABRICANTE, M.MODELO_TIPO_AUTO, M.MODELO_TIPO_CAJA, M.MODELO_TIPO_MOTOR, M.MODELO_TIPO_TRANSMISION, M.MODELO_POTENCIA, CL.CLIENTE_FECHA_NAC
...
SELECT MES.ID, F.FACTURA_SUCURSAL, 
	(CASE WHEN DATEDIFF(YEAR,CL.CLIENTE_FECHA_NAC,GETDATE()) BETWEEN 18 AND 30 THEN (SELECT TOP 1 A.ID FROM NAPOLITANA_CON_FRITAS.BI_Cliente A WHERE A.SEXO = 'NO INFORMA' AND A.RANGO_EDAD = '18 - 30 años')
		WHEN DATEDIFF(YEAR,CL.CLIENTE_FECHA_NAC,GETDATE()) BETWEEN 31 AND 50 THEN (SELECT TOP 1 A.ID FROM NAPOLITANA_CON_FRITAS.BI_Cliente A WHERE A.SEXO = 'NO INFORMA' AND A.RANGO_EDAD = '31 - 50 años')
		WHEN DATEDIFF(YEAR,CL.CLIENTE_FECHA_NAC,GETDATE()) > 50 THEN (SELECT TOP 1 A.ID FROM NAPOLITANA_CON_FRITAS.BI_Cliente A WHERE A.SEXO = 'NO INFORMA' AND A.RANGO_EDAD = '> 50 años')
		ELSE NULL
...
JOIN NAPOLITANA_CON_FRITAS.Modelo AS M ON A.AUTO_MODELO = M.MODELO_CODIGO
JOIN NAPOLITANA_CON_FRITAS.Cliente AS CL ON CL.CLIENTE_ID = F.FACTURA_CLIENTE
GROUP BY MES.ID, F.FACTURA_SUCURSAL, M.MODELO_CODIGO, M.MODELO_FABRICANTE, M.MODELO_TIPO_AUTO, M.MODELO_TIPO_CAJA, M.MODELO_TIPO_MOTOR, M.MODELO_TIPO_TRANSMISION, M.MODELO_POTENCIA, CL.CLIENTE_FECHA_NAC
