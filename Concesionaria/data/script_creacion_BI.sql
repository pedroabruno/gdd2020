...
PRIMARY KEY (MES_ID,SUCURSAL_ID,CLIENTE_ID,MODELO_ID,FABRICANTE_ID,TIPO_AUTO_ID,TIPO_CAJA_ID,TIPO_MOTOR_ID,CANTIDAD_CAMBIOS_ID,TIPO_TRANSMISION_ID,POTENCIA_RANGO_ID),
...
INSERT INTO NAPOLITANA_CON_FRITAS.BI_Compra_Auto (MES_ID,SUCURSAL_ID,CLIENTE_ID,MODELO_ID,FABRICANTE_ID,TIPO_AUTO_ID,TIPO_CAJA_ID,TIPO_MOTOR_ID,TIPO_TRANSMISION_ID,POTENCIA_RANGO_ID,PRECIO,CANTIDAD,CANTIDAD_CAMBIOS_ID)
SELECT MES.ID, C.COMPRA_SUCURSAL, CL.cliente_edad, M.MODELO_CODIGO, M.MODELO_FABRICANTE, M.MODELO_TIPO_AUTO, M.MODELO_TIPO_CAJA, 
	...
	(CASE WHEN M.MODELO_POTENCIA BETWEEN 50 AND 150 THEN (SELECT TOP 1 P.ID FROM NAPOLITANA_CON_FRITAS.BI_Potencia_Rango P WHERE P.RANGO = '50-150cv')
		...
		ELSE NULL
	END)
	, AVG(IC.ITEM_PRECIO), SUM(IC.ITEM_CANTIDAD), 3
FROM NAPOLITANA_CON_FRITAS.Compra AS C
...
INSERT INTO NAPOLITANA_CON_FRITAS.BI_Venta_Auto (MES_ID,SUCURSAL_ID,CLIENTE_ID,MODELO_ID,FABRICANTE_ID,TIPO_AUTO_ID,TIPO_CAJA_ID,TIPO_MOTOR_ID,TIPO_TRANSMISION_ID,POTENCIA_RANGO_ID,PRECIO,CANTIDAD,DIAS_PROMEDIO_STOCK,CANTIDAD_CAMBIOS_ID)
SELECT MES.ID, F.FACTURA_SUCURSAL, CL.cliente_edad, M.MODELO_CODIGO, M.MODELO_FABRICANTE, M.MODELO_TIPO_AUTO, M.MODELO_TIPO_CAJA, 
	...
	(CASE WHEN M.MODELO_POTENCIA BETWEEN 50 AND 150 THEN (SELECT TOP 1 P.ID FROM NAPOLITANA_CON_FRITAS.BI_Potencia_Rango P WHERE P.RANGO = '50-150cv')
		...
		ELSE NULL
	END)
	, AVG(ItF.ITEM_PRECIO), SUM(ItF.ITEM_CANTIDAD), AVG(Stock.DIAS_STOCK), 3
FROM NAPOLITANA_CON_FRITAS.Factura AS F
...
SELECT V.MODELO_ID, AVG(V.PRECIO) AS PROMEDIO_VENDIDO, AVG(C.PRECIO) AS PROMEDIO_COMPRADO, AVG(V.DIAS_PROMEDIO_STOCK) AS DIAS_PROMEDIO_EN_STOCK
FROM NAPOLITANA_CON_FRITAS.BI_Venta_Auto AS V
JOIN NAPOLITANA_CON_FRITAS.BI_Compra_Auto AS C ON C.MODELO_ID = V.MODELO_ID
...
SELECT V.AUTOPARTE_ID, AVG(V.PRECIO) AS PROMEDIO_VENTA, AVG(C.PRECIO) AS PROMEDIO_COMPRA
FROM NAPOLITANA_CON_FRITAS.BI_Venta_Autoparte AS V
JOIN NAPOLITANA_CON_FRITAS.BI_Compra_Autoparte AS C ON C.AUTOPARTE_ID = V.AUTOPARTE_ID
